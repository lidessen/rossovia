...
owner: "unknown"
consumer: harness-iteration-methods
review_at: next-real-project-scale-method-evolution-wave
---
...
## Settlement event（2026-08-26；历史结算）

本研究已将“以可证伪改变、独立证据、回退/保留和采用后回归形成净改善”的方法 proposal 交给现行
`iterative-improvement` theory 与 planning/application surfaces，当前结算为 `settled / canonical-proposal`。
现有证据仍不支持 project-scale matched improvement、长期 adoption 或 regression；这些是未来真实 consumer
出现后的 reopen 条件，而不是本研究继续无限扩张的理由。

随后用户明确提出 goal-scale、长期执行的工作方法演化 consumer；该新 consumer 触发本记录按 §10 reopen，
不倒写本历史结算。

## Reopen：goal 驱动的工作方法持续演化候选（2026-08-26）

用户新增了一个 decision-changing consumer：goal 驱动或无限执行的 Agent 不只在任务结果上迭代，
还要在工作推进到相称程度后回顾“当前工作方法是否有效”，保留已经证明有用的方法，修订或淘汰导致
重复问题、退化或无效成本的方法。这个输入不是要求 Agent 自己随意改写 harness，也不是要求每隔固定
轮数运行一个自我评价器；它把本研究的对象从“单轮 change”扩展为一个有 scope 的**方法演化关系**。

### 两层回路

```text
任务执行回路：目标/方法快照 → 行动 → 观察/纠偏 → work-map 前进
                                      │
                         语义 checkpoint / safe point
                                      ▼
方法演化回路：收集结果 → 回顾方法 → change hypothesis
             → 有界比较/独立 review → retain / adapt / adopt / rollback / no-proposal
             → 新方法快照 + lineage → 返回任务执行回路
```

任务执行回路追求当前目标的有效进展；方法演化回路追求“产生进展的方法”本身的可复用性和可靠性。
后者不能从前者的单次成功直接推得。若任务目标、consumer、owner、权限、共享基线或接受关系改变，
先结束旧方法快照的有效范围，再建立新的方法对象，不能把不同目标的经验混为一个长期自我。

### 回顾何时发生

回顾触发由会改变注意关系或下一项判断的语义边界决定，并以工作量上限提供 liveness 保护；不规定
全局固定轮数、调用次数或 token 阈值。候选触发包括：

- goal 的一个阶段、里程碑、交付物或主要矛盾已经改变；
- 同类错误、用户纠正、回退、等待或协调浪费重复出现；
- context 压缩、角色/consumer handoff、进程恢复或长时间暂停后重新接续；
- 方法的 outcome 没有改善，或 process/balancing cost（时间、token、等待、维护）开始侵蚀目标；
- 进入 owner return、重大决定、采用前或回归观察窗口；
- 当前工作量已经达到该 goal 预先给出的下一次可检查 safe point，即使没有明显失败，也只做轻量回顾。

最后一项不是“每 N 步强制反思”：goal 只需提供一个相称的最大回顾间隔，实际回顾仍应在最近的安全
边界完成；低风险、一步且无方法差距的工作可以记为 `N/A`，不能为了满足间隔制造改动。

### 回顾看什么，以及怎样判断好坏

回顾对象是当前**方法快照**，不是只看最终文件。最小观察向量包括：

| 观察面 | 要回答的问题 | 典型负面信号 |
| --- | --- | --- |
| outcome | 方法是否帮助目标任务取得预期进展或修复主要缺口？ | 目标没有前进、重复返工、错误交付 |
| boundary | 是否保住来源、权限、owner、unknown 和安全边界？ | 越权、丢 lineage、把候选写成接受、回归逃逸 |
| process | 是否更早发现问题、正确选择下一步并完成 handoff/review？ | 误读 authority、重复调查、错误委派、无法回接 |
| balancing | 方法增加了多少 token、时间、等待、协调和维护负担？ | 复述泛滥、review 仪式化、并行抵消、记录膨胀 |
| transfer | 观察是否能在另一个相近场景重建并改变判断？ | 只对单一任务有效、依赖偶然上下文或作者记忆 |

“好方法”不是 Agent 自评满意，而是已有 baseline/目标关系在相称正例、边界和回归观察中保持，且
下一判断更可靠或成本更低；“差方法”也不等于一次失败，而是重复失败、硬边界退化、无法回接、成本
不值得或没有可证伪收益。证据不足时返回 `uncertain`，不强迫更新。

### 方法更新的四个范围

1. **局部纠偏：** 只改变当前任务的下一步或 focus anchor；可观察、可回退时由 Main 自主处理，不形成
   方法版本。
2. **goal-scoped snapshot：** 在同一 goal/role/consumer 范围内，经过一次有界比较或相称 review 后，
   更新当前方法快照；保留旧快照、理由、影响范围和 rollback anchor。
3. **reusable method candidate：** 同类差距跨多个相近任务重复出现，才提出 skill、workflow 或理论
   candidate；需要独立 review、边界/回归和真实 consumer，不由单次 goal 自动推广。
4. **shared/runtime change：** 若改变共享协议、权限、持久身份、并发、恢复、外部效果或长期默认行为，
   交给对应 owner/acceptance；文字中的方法更新不能取得 runtime guarantee。

### 最小方法快照与更新记录

方法演化需要保留一个可回读的当前 snapshot，但不需要全局 method registry。最小内容是：

```yaml
scope: goal / role / consumer
baseline: source and method artifact references
current_method: the methods actually used for this scope
observations: outcome / boundary / process / balancing / transfer
change_hypothesis: one main semantic delta and falsifier
guardrails: hard constraints, stop and rollback conditions
review_trigger: next semantic checkpoint or bounded safe-point window
disposition: retain-baseline / adapt-and-retest / adopt-local / rollback / no-proposal / uncertain
lineage: supersedes / derived-from / affected downstream views
```

这不是新的公共 schema 或状态机；字段是为了让 Main 能重建“当前用的是什么、为什么改、改坏如何回去”。
`disposition` 是本轮证据处置，不是 lifecycle `status`；历史更新追加到 record，不覆盖旧 snapshot。

### 防止自我强化和无限抖动

- 不允许候选同时改方法、验收标准和 holdout，再用改后的标准证明自己成功；验收关系由外部 owner 或独立
  reviewer 保留。
- 保留 frozen baseline、候选快照和 rollback anchor；退化时先回到已知 baseline，再形成下一 hypothesis。
- development 正例、boundary/nearest-owner、regression 和 fresh holdout 分层；反复看过的任务不能继续冒充
  holdout。
- 同一个局部观察最多先改变其最小 scope；跨任务推广必须有 transfer evidence，跨 harness 推广必须重新
  判断 source、owner、风险和维护收益。
- 新快照只有在它能改变下一判断或减少可观察成本时才被保留；没有 decision delta 的回顾只留下
  `retain-baseline / no-proposal`，不制造版本、文件或 Agent。

### 当前证据与最小下一实践

本节目前是 `design-candidate / source-bounded / no-project-run / acceptance-pending`。它由既有
iterative-improvement proposal、practice-cycle、default-autonomy-with-correction、planning-information-architecture
和用户新增 consumer 推导而来；尚无 goal-scale matched comparison、长期运行、独立 review 或 adoption
证据，不授权 self-improvement registry、scheduler、自动改写器或 runtime mechanism。

下一项最小实践应在一个真实的多步骤 goal wave 中冻结：

1. 一个初始方法快照和 frozen baseline；
2. 至少一个语义 checkpoint 及其触发原因；
3. 一条方法 change hypothesis 和 rollback 条件；
4. 同一任务族中的 adaptive-method 与 frozen-method 对照（无法匹配时降级为 bounded observation）；
5. outcome、boundary、process、balancing、transfer 和独立 review 结果。

在真实 consumer、runner、reviewer、接受关系和可重建记录形成前，只保留该 design candidate；不把
“持续自我迭代”写成当前 harness 已具备的能力。
