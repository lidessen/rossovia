...
或一组任意语言输入，保留可重建的原话、顺序、来源与上下文，形成可审查的秘书
...
`inferred`、`unknown` 是可恢复和可审查的区分，不是字段合同。原话可以按语义
转折拆成多个片段，但应保留批次、顺序、逐字内容、来源和上下文，使原话可重建；
不得用摘要覆盖或无标记地纠正它。一个 raw 可以有多个解释候选，但候选不因此
...
收到任意语言后，首先保留可回读的 raw source。原话不必物理上保持一整块：可以
按语义转折拆成片段，但要保留批次、顺序、逐字内容、来源与上下文，使原话可重建。
秘书可以做轻量的转述、分段、聚类、提取问题和标记可能的关系，但不得用摘要覆盖、
改写后删除或无标记地纠正原话。若 source 后来被用户修改、撤回或纠正，应保留
...
blocker、owner 或授权。秘书应忠实呈现弱语气，而不是替用户增强它。用户明确
...
Markdown 边界内，可以把未来 `planning/inbox.md` 作为简洁的 raw source 或其
...
本理论不是行为已成立的证明。格式正确、存在原话、一次成功整理、checkbox 全勾、
...
- 混合想法、观察、疑问、todo、半计划和明确命令，检查 raw fidelity 以及
