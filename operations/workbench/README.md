...
one of its recorded previous attempts, pass that session id explicitly with
`--session <id>`. Workbench reads the owner-backed Work Cell final record before
forwarding the session to the Work Cell CLI; a session absent from this active
Task's recorded attempts, or one that does not match the session observed in
the returned final record, fails. Every run result reports the OpenCode session
id actually observed in that attempt's final Work Cell record, whether or not a
session was requested. The Workbench retains the
input, final Work Cell record, and a small append-only settlement under its home
state. Cell settlement is execution evidence only: it does not submit or accept
the Task. A completed Task is ordinary viewable history and cannot run.
...
## Project identity status lines

The status line answers one question: **which project is this Agent handling?**
It deliberately excludes task queues, branch state, model/runtime telemetry,
and multi-Agent detail; those belong to Workbench or the host's Agent view.
...
label. Without one, the renderer falls back to a uniquely registered project
ID, then the Git-root or directory basename. It never prints an absolute path
and does not create project, task, session-focus, or runtime state.

Claude Code uses its documented command-backed `statusLine` surface in
`../../.claude/settings.json`. The adapter displays Claude's `session_name` and
uses the repository fallback only before a name exists. Name a vague session
with `/rename <project>` so several tabs opened from this Workbench root remain
distinguishable. Multi-Agent rows remain in Claude's native Agent panel. See
the [Claude Code status-line contract](https://code.claude.com/docs/en/statusline).

Codex does not support a command-backed renderer, but it exposes the current
thread name as native `thread-title` status-line and `thread` terminal-title
items. The project-local `../../.codex/config.toml` selects only those items;
use `/rename <project>` when needed. Codex's native Agent switcher continues to
own multi-Agent detail. See the
[Codex configuration reference](https://learn.chatgpt.com/docs/config-file/config-reference)
and use `/statusline` or `/title` inside Codex to inspect the projections.
