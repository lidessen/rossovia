 ...
 * The adapter owns the protocol's exit/stderr refusal, output trim and the
 * non-empty requirement, path resolution, and realpath normalization;
 * exposing this raw outcome as an explicit call-bound invocation seam lets
 * a caller or test supply one exact outcome so those policy branches are
 * deterministically discriminated without intercepting PATH.
 */
export interface GitRevParseInvocationOutcome {
  /** The process exit status; null when the process never started. */
 ...
 * process exit/stderr refusal, output trim and the non-empty requirement,
 * the relative/absolute git-dir resolution against the Worktree, and the
 * final realpath normalization. Orchestration consumes the neutral
 * `GitWorktreeMetadataPort`; it never constructs this invocation itself.
 * The raw invocation is an explicit call-bound seam: production always uses
 * the declared `spawnSync` invocation, and only an explicit argument may
 * substitute one exact outcome for this call.
 */
