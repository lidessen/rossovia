...
export function topLevelUsage(): string {
  const lines = [
    ...
    ...HELP.filter((entry) => entry.topLevel)
      .map((entry) => `  ${entry.kind === "family" ? entry.summary : entry.usage}`),
    "",
    "run 'rossovia help <command>' for per-command usage",
    "rossovia --version prints the @rosso/workbench package version",
    "",
  ];
  return lines.join("\n");
}
