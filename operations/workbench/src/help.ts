...
export const HELP: HelpEntry[] = [  { kind: "verb", path: ["init"], topLevel: true,
  ...
  { kind: "family", path: ["hook"], topLevel: true,
    ...
    notes: "Platform validation is the usage boundary: a missing or invalid platform exits 2 with a help pointer before any payload read or state write; after a valid platform, later failures keep that platform's exit-0 fallback. Both verbs read one JSON payload from stdin on every non-help invocation." },
  ...
  { kind: "verb", path: ["hook", "artifact"], topLevel: false,
    ...
    notes: [
      ...
      "state: appends only repository-relative relevant paths (skills/**/SKILL.md, skills/**/commands|references/*.md, AGENTS.md, CLAUDE.md, README.md) to $TMPDIR/rossovia-hooks/artifact-consistency/<40-hex>.jsonl keyed by repository root, platform, and session. This is operating-system temporary state, not Workbench home state; payloads without relevant changed paths are a silent no-op.",
      "output: post-tool-use and after-file-edit succeed silently; a stop with recorded paths returns one one-shot reminder — codex/claude {\"systemMessage\":...} and removes that session's file, cursor {\"followup_message\":...} and keeps it — and a cursor stop with loop_count>0 is silent and continues the active run. Repeated stops re-notify nothing; decision and additionalContext are never returned.",
