...
describe("Mission anchor migration proposal", () => {
  ...
  test("verifies the exact committed Mission source and rejects file or HEAD drift", async () => {
    ...
    const relativePath = "operations/missions/legacy-mission.json";
    await mkdir(join(sourceRoot, "operations", "missions"), { recursive: true });
    git(sourceRoot, "init");
...
async function fixture() {
  ...
  const proposal = MissionAnchorMigrationProposalSchema.parse({
    ...
    missionSource: {
      projectId: "project-1",
      relativePath: "operations/missions/legacy-mission.json",
      gitHead: "a".repeat(64),
    },
    target: {
