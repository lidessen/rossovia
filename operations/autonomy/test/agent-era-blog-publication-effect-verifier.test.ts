...
async function createPublicationCandidate(root: string): Promise<void> {
  ...
  await symlink(join(BLOG_ROOT, "node_modules"), join(root, "node_modules"), "dir");
  ...
  run(root, "npm", ["run", "db:generate", "--", "--name", "seeded_publication"]);
}
