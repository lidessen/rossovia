...
export async function verifyAgentEraBlogPublicationCandidate(
  ...
  try {
    ...
    const checks = [
      ...
      commandCheck({
        id: "build",
        command: "npm run build (temporary publication candidate snapshot)",
        executable: "npm",
        arguments: ["run", "build"],
        cwd: snapshot,
      }),
      commandCheck({
        id: "test",
        command: "npm test (temporary publication candidate snapshot)",
        executable: "npm",
        arguments: ["test"],
        cwd: snapshot,
      }),
      commandCheck({
        id: "migration",
        command: "npm run db:generate (temporary publication candidate snapshot)",
        executable: "npm",
