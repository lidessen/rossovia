...
const MISSION_SOURCE = "apps/missions/principal-workbench-dogfood.json";
const RUNTIME_REF = "source-project:apps/autonomy/experiments/agent-era-blog-mission-runtime.ts";
...
export const createMissionRuntime: MissionRuntimeFactory = async (
  ...
  const delegate = new DelegateLoopSession({
    ...
    // The supervised trial owns the real process: it explicitly injects the
    // local host adapter over its operator-selected isolated worktree.
