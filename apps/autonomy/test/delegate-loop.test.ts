...
test("skipping worker_list still shows the deepseek-flash default selection policy in catalog mode", async () => {
  ...
  // the default selection policy: prefer deepseek-flash (reasoning=max) for
  // ordinary engineering work, explicit architecture/high-difficulty or
  // visual exceptions, worker_list first when uncertain, and no host
  // replacement of the explicitly selected workerId.
  expect(calls).toBe(2);
