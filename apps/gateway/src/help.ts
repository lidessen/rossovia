import { UsageError } from "../../workbench/src/cli-errors";
...
export const HELP: HelpEntry[] = [  { kind: "verb", path: ["init"], topLevel: true,
  ...
  { kind: "verb", path: ["task", "create"], topLevel: true,
    usage: "task create --title <text> --objective <text> --accept <criterion>... [--todo <text>]... [--capability-required <label>]... --next-actor <principal|agent|external> --source-ref <reference> --expected-source-revision <n> [--project <project> [--worktree <path>] [--mission <id>]]",
    description: "Create a Principal-attributed task from an explicit current request.",
    effect: "writes-state",
    notes: "Optional --capability-required <label> repeats exact WorkerCatalog capability labels the task requires of any selected worker. Labels must use lowercase letters, digits, and hyphens and must be unique; they are persisted verbatim and lowered unchanged into CellInput.capabilitiesRequired on task run, and are never inferred from the objective, acceptance, or final text. Omitting the option creates the task with an empty requirement list." },
  ...
  { kind: "verb", path: ["task", "run"], topLevel: true,
    usage: "task run <id> --worker <worker-id> [--continue <attempt-id>] [--read-only] [--enable-observer --observer <worker-id>]",
    description: "Run an open project task in its exact worktree with the selected catalog worker; --continue starts a stateless continuation of the exact named prior attempt lineage; --read-only runs the same task through the explicit read-only entry.",
    effect: "starts-work",
    notes: "Optional --max-steps <positive-integer> is retained in the canonical Run request and digest and lowered into the immutable CellInput budget; when omitted the budget has no step limit and only the 30-minute emergency timeout remains. Optional --read-only carries RunRequest access read-only into the canonical request digest, lowers an immutable CellInput with no write paths and no allowed commands, and never acquires the O3 writer claim; the omitted default stays the ordinary effectful run. Optional --enable-observer --observer <worker-id> starts one detached read-only workflow review after terminal settlement and appends one local review record." },
  ...
  { kind: "family", path: ["intervention"], topLevel: false,
    ...
    subcommands: ["observe", "correct", "status"],
    notes: "Intervention state lives under a state root: <home>/state/interventions by default (home = leading --home or ROSSO_HOME), overridden by --state-root. observe writes prompt evidence, correct appends a Principal correction, and status reads the intervention-local append-only record projection." },
  ...
  { kind: "verb", path: ["intervention", "status"], topLevel: true,
    ...
    notes: [
      ...
      "recovery: when no state exists yet, the nearest recovery is rossovia intervention observe (it creates the state); use rossovia intervention correct --state-file <path> to append a receipt.",
    ].join("\n") },
  { kind: "verb", path: ["intervention", "correct"], topLevel: true,
    usage: "intervention correct --state-file <path> --rejected-assumption <text> --new-invariant <text> --affected-surface <name>... --next-probe <text>",
    description: "Append one Principal correction to an intervention state and return its intervention-local record projection.",
    effect: "writes-state",
    notes: [
      ...
      `output: {"statePath":..., "receipt":{...}, "record":{...}} on stdout, exit 0. A missing state file exits 1 — recover with rossovia intervention observe first.`,
    ].join("\n") },
  ...
  { kind: "verb", path: ["ui"], topLevel: true,
    usage: "ui [--port <port>] [--root <path>]... [--observer <worker-id>] [--disable-observer]",
    description: "Serve the Rossovia Principal Workbench web UI on 127.0.0.1.",
    effect: "starts-work",
    notes: [
      ...
      "defaults: port 4317; running from the source checkout observes one --root defaulting to the current repository root, while a compiled install starts with no default root and observes only explicit --root entries; repeat --root to add more observed roots. Local startup enables one deepseek-flash observer per settled Task attempt (a conversation task_continue settles one such attempt); use --observer <worker-id> to choose another worker or --disable-observer to turn it off. A plain conversation Run settles no Task attempt and is not observed (query gap). The UI shares the CLI's typed local-task control-plane port and never binds a non-loopback address.",
  ...
  { kind: "verb", path: ["observer"], topLevel: true,
    usage: "observer --attempt <attempt-id> --worker <worker-id>",
    description: "Run one detached workflow review against a settled project attempt using the standard read-only evidence path.",
    effect: "starts-work",
    notes: "The observer is project-agnostic, optional, and non-authoritative: it does not create a Task lifecycle, write the Worktree, retry, accept, merge, or roll back. Its single review opinion is appended to the local workflow review log." },
  { kind: "verb", path: ["self-check"], topLevel: true,
    ...
    effect: "starts-work",
    notes: "Mechanical checks own health facts and include source references. The startup gate is explicitly scoped to boot: it reports readiness=boot-ready|boot-attention and projection=not-checked; a later /api/snapshot complete value is a separate projection and is not implied by startup readiness. --task reads one existing Principal Task/Todo through the canonical read port and re-reads its revision after the opinion; no synthetic Task is created. The current host has no transient Task subscription, so the result labels snapshot-only standing and changes as stale. --progress streams checking and terminal item states to stderr without writing state. --opinion is optional and non-authoritative: without --task it reports a Task/Todo query-gap and does not start a worker; provider unavailability or timeout never blocks mechanical boot readiness or changes mechanical evidence." },
