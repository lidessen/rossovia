...
export const HELP: HelpEntry[] = [  { kind: "verb", path: ["init"], topLevel: true,
  ...
  { kind: "verb", path: ["ui"], topLevel: true,
    ...
    notes: [
      ...
      "defaults: port 4317, one --root defaulting to the current repository root; repeat --root to add more observed roots. The UI shares the CLI's typed local-task control-plane port and never binds a non-loopback address.",
  ...
  { kind: "verb", path: ["observer"], topLevel: true,
    ...
    description: "Run one detached dogfood review against a settled attempt using the standard read-only evidence path.",
    effect: "starts-work",
    notes: "The observer is optional and non-authoritative: it does not create a Task lifecycle, write the Worktree, retry, accept, merge, or roll back. Its single review opinion is appended to the local dogfood review log." },
