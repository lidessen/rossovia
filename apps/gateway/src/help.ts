...
export const HELP: HelpEntry[] = [  { kind: "verb", path: ["init"], topLevel: true,
  ...
  { kind: "verb", path: ["ui"], topLevel: true,
    ...
    notes: [
      ...
      "defaults: port 4317; running from the source checkout observes one --root defaulting to the current repository root, while a compiled install starts with no default root and observes only explicit --root entries; repeat --root to add more observed roots. Local startup enables one deepseek-flash observer per settled conversation Run; use --observer <worker-id> to choose another worker or --disable-observer to turn it off. The UI shares the CLI's typed local-task control-plane port and never binds a non-loopback address.",
