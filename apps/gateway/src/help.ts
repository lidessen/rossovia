...
export const HELP: HelpEntry[] = [  { kind: "verb", path: ["init"], topLevel: true,
  ...
  { kind: "verb", path: ["self-check"], topLevel: true,
    usage: "self-check [--cwd PATH] [--baseline-head SHA] [--trigger manual|startup|changed] [--progress] [--opinion --worker <worker-id> --timeout-ms MS]",
    description: "Run a fast read-only mechanical preflight and optionally request a bounded worker opinion.",
    effect: "read-only",
    notes: "Mechanical checks own health facts and include source references. --progress streams checking and terminal item states to stderr without writing state. --opinion is optional and non-authoritative: it runs only after mechanical preflight, returns an opinion/attention standing with confidence and evidence refs, and provider unavailability or timeout never blocks startup or changes mechanical evidence." },
