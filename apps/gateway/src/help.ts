...
export const HELP: HelpEntry[] = [  { kind: "verb", path: ["init"], topLevel: true,
  ...
  { kind: "verb", path: ["task", "run"], topLevel: true,
    usage: "task run <id> --worker <worker-id> [--continue <attempt-id>]",
    ...
    effect: "starts-work",
    notes: "Optional --max-steps <positive-integer> is retained in the canonical Run request and digest and lowered into the immutable CellInput budget; when omitted the budget has no step limit and only the 30-minute emergency timeout remains." },
