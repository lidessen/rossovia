...
export const HELP: HelpEntry[] = [  { kind: "verb", path: ["init"], topLevel: true,
  ...
  { kind: "verb", path: ["task", "run"], topLevel: true,
    ...
    description: "Run an open project task in its exact worktree with the selected catalog worker; --continue starts a stateless continuation of the exact named prior attempt lineage.",
    effect: "starts-work",
    notes: "Optional --max-steps <positive-integer> is retained in the canonical Run request and digest and lowered into the immutable CellInput budget; when omitted the budget has no step limit and only the 30-minute emergency timeout remains. Optional --enable-observer --observer <worker-id> starts one detached read-only workflow review after terminal settlement and appends one local review record." },
