...
export const HELP: HelpEntry[] = [  { kind: "verb", path: ["init"], topLevel: true,
  ...
  { kind: "verb", path: ["task", "run"], topLevel: true,
    usage: "task run <id> --worker <worker-id> [--continue <attempt-id>] [--enable-observer --observer <worker-id>]",
    ...
    effect: "starts-work",
    notes: "Optional --max-steps <positive-integer> is retained in the canonical Run request and digest and lowered into the immutable CellInput budget; when omitted the budget has no step limit and only the 30-minute emergency timeout remains. Optional --enable-observer --observer <worker-id> starts one detached read-only workflow review after terminal settlement and appends one local review record. Optional --read-only carries RunRequest access read-only into the canonical request digest, lowers an immutable CellInput with no write paths and no allowed commands, and never acquires the O3 writer claim; the omitted default stays the ordinary effectful run." },
