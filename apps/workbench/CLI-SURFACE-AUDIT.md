...
| mission | `mission list` exit 0 JSON from repo root (reads `apps/missions`); cwd-dependent; `--root` must precede subcommand ([missions.ts:312](src/missions.ts)) — superseded by the T4 implementation update below | `mission --help`, `mission list --help` exit 2 | branch mutations |
...
- The default mission root `<cwd>/apps/missions` (resolved to an
...
- Named checks (same order as
  ...
  `packages/work-cell`, `packages/cognition`, `apps/autonomy`,
  `apps/workbench`; `bun run typecheck` (0 errors) and `bun test` (496
  pass, 0 fail) in `apps/workbench`; `typecheck` green in the two
