...
**Status:** development run completed but invalid; see [RESULT.md](RESULT.md)
...
These commands document the executed packet. Do not run the final command again
under the old authorization; any retry must be a newly hashed and reviewed
packet.

From `packages/work-cell`:

```bash
cd ../../regeneration/evaluations/evidence/2026-08-06-todo-obligation-carrier
...
The authorized packet ran once. The retained [result](RESULT.md) is invalid for
the preregistered H4 comparison because the observer parsed the wrong tool-call
surface, neither arm maintained the frozen read-then-update Todo cadence, and
the manifest's declared semantic-audit digest does not identify the retained
audit bytes. The frozen manifest and launch record are preserved rather than
rewritten after execution.
The exact pre-run README is preserved as
[`README.pre-run.md`](README.pre-run.md) in its original relative-link context.
Do not rerun under the old authorization.
