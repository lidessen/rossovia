...
当前工程决策应是：**KB 的机器索引层以带类型、可检索的文本关系为准；完整关系图仅作为
按需生成的投影，不能替代索引。**

这不是说 Agent 看不懂关系图。本轮图像条件中，直接边、反向负例、不存在的边、关系标签
和四跳关系链全部答对。问题出现在高密度图的完备枚举：相同的 15 个节点下，图像准确率
随边数从 13、22 到 34 条，由 100% 降到 87.5% 和 75%；中密度开始持续漏算入边，
高密度又持续漏掉一条出边。对索引层而言，“大致理解”不够，漏掉一条关系就是错误索引。

文本条件在同题、同模型、同顺序的 9 次运行中为 72/72。但它是“完整文本附件”控制组，
还不是带搜索工具的真实 retrieval 画像。因此它足以否定“完整图像可直接替代文本”，尚不足以
量化真实文本检索相对任务相关子图的最终成本优势。
...
这些论文和本地实验共同支持一种分层，而不是二选一：文本/结构化关系保存机器事实；搜索先
裁剪任务相关子图；图像用于全局感知、人类沟通或特定视觉模式；需要模型同时使用时，把文本
事实与派生子图并列提供，但不让图片成为唯一事实源。
...
下一轮最有价值的对照不是继续放大整图，而是：

1. 冻结一组 held-out 图和第二种布局；
2. 比较真实 `search → top-k 文本关系`、`search → 任务相关子图 PNG`，以及把相同关系
   文本渲染成紧凑文档图的 optical-compression 条件；
3. optical-compression 条件分别测“恢复文本”与“直接回答”，不把 OCR 精度当推理精度；
4. 保持模型、查询预算和返回事实集合一致，分别记录检索、渲染、模型调用的时延与成本；
5. 至少加入第二个多模态模型，验证 incident-edge 漏检是否是路线特性。

在这些反证测试通过前，结论保持 `probe`，默认实现仍是文本索引 + 可再生可视化投影。
