...
export interface ValidationModelOptions {
  ...
  deepSeekInferencePolicy?: DeepSeekInferencePolicy;
  deepSeekStructuredOutputMode?: "inline" | "tool-settlement";
  kimiApiKey?: string;
  kimiBaseURL?: string;
  kimiModel?: string;
...
export function createValidationModel(
    ...
    if (target.provider === OPENCODE_GO_PROVIDER_ID) {
      ...
      targets.push({
        ...
        model: createDeepSeekModel({
          ...
          model: target.model,
          ...(target.baseURL ? { baseURL: target.baseURL } : {}),
          ...(options.deepSeekInferencePolicy
            ? { inferencePolicy: options.deepSeekInferencePolicy }
            : {}),
        }),
  ...
  return {
    ...
    structuredOutputMode: resolvedRoute.every((target) => (
      target.provider === OPENCODE_GO_PROVIDER_ID
      || (
        target.provider === DEEPSEEK_PROVIDER_ID
        && options.deepSeekStructuredOutputMode === "inline"
      )
      || (
...
function validationPricing(
  ...
  if (models[0] === "deepseek-v4-flash-vision-exp") {
    return { pricing: deepSeekFlashVisionExpPricing };
  }
