...
| `refine` | command, host direction/system, rendered evidence, component-expression reference | component-system reference when a detected open-code system changes ownership or comparison mechanics; report a route change when structure, direction, or a primary production asset must be reconsidered |
